...
export default function LoginScreen({ onLogin, onGoogleLogin }) {
  ...
  const handleGoogleResponse = useCallback((response) => {
    if (response?.credential) {
      ...
      setError('');
      const result = onGoogleLogin(response.credential);
      if (!result.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = onLogin(mode, username.trim(), password, displayName.trim());
          ...
          <div className="bg-white rounded-3xl p-6 shadow-2xl flex-1 flex flex-col overflow-y-auto scrollbar-hide">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col flex-1">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className={`w-full h-11 rounded-xl font-bold text-[14px] flex items-center justify-center gap-2 transition-all mt-auto ${
                  isLoading
